# Universal MCP Server Recommendations

MCP (Model Context Protocol) servers extend AI Agent capabilities (Claude Code, Antigravity, Codex, etc.) by connecting to external tools, APIs, and services.
...
- **Claude Code**: Project config (`.mcp.json`) or global config (`~/.claude.json`). Use `claude mcp add`.
- **Antigravity (Gemini)**: Defined in `.gemini/config/plugins/...` or tool manifest.
- **Codex**: Standardized MCP server config or external tool declarations.

**Tip**: Check `.mcp.json` into git so your whole team gets the same MCP servers across compatible agents.
