# Universal Plugin & Extension Package Recommendations

Plugins (and Extension Bundles) are installable collections of skills, commands, subagents, and hooks.

**Plugin System by Agent Platform:**
- **Claude Code**: Official plugin repository (install via `/plugin install <name>`) or `.claude/plugins/`.
- **Antigravity (Gemini)**: Plugin folders located under `.gemini/config/plugins/` (exposing skills, agents, and configs).
- **Codex**: Custom skill/script repositories or environment package extensions.

**Note**: These are common plugins. Use web search to discover additional community plugins.
