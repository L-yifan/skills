# Universal Skills & Tools Recommendations

Skills (and Custom Tools) are packaged expertise containing workflows, reference materials, templates, and scripts. 

**Path Locations by Agent Platform:**
- **Claude Code**: `.claude/skills/<name>/SKILL.md` (invoked via `/skill-name` or model context)
- **Antigravity (Gemini)**: `.gemini/config/plugins/.../skills/<name>/SKILL.md` or workspace skills
- **Codex**: Executable Python/Shell scripts in `tools/` or `scripts/`, described in `AGENTS.md`

Some pre-built skills are available through official plugins or agent skill packages.
