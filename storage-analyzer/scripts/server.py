    ...
    def do_POST(self):
        ...
        for p in (req.get("paths") or []):
            ...
            if rp not in allow:
                ...
                return
            # 二级护栏：
            # 破坏性删除操作（rm / trash）严格限制在 HOME 用户目录内，绝不允许触碰系统其他位置；
            # 非破坏性操作（open 在资源管理器中定位查看）已受 OPEN_ALLOW 报告白名单约束，允许打开审查。
            if mode in ("rm", "trash"):
                if not (rp == HOME or rp.startswith(HOME + os.sep)):
                    self._send(403, json.dumps({"ok": False, "error": "删除路径越界：%s" % p}))
                    return
            try:
                if mode == "open":
...
def main():
    ...
    print("绿灯可删 %d 项 | 橙灯可移废纸篓/打开文件夹 %d 项 | 页面上点" % (len(RM_ALLOW), len(TRASH_ALLOW) - len(RM_ALLOW)), flush=True)
