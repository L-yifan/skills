...
show_usage() {
  echo "Usage: ./init-artifact.sh <project-name> [--framework react|vue]"
  echo "   or: ./init-artifact.sh --framework react|vue <project-name>"
}
...
if [ "$FRAMEWORK" = "vue" ]; then
  echo "🔀 Delegating to Vue initializer..."
  bash "$SCRIPT_DIR/init-artifact-vue.sh" "$PROJECT_NAME"
  exit $?
fi
